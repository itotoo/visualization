            ...
            <React.Fragment>
                <div className="nav">
                {
                    Object.keys(types).map((item , key) =>  
                        <NavLink to={{ pathname: `/animation/${item}` ,search:`?name=${item}`}} activeClassName="active" replace key={key}> {item}</NavLink>
                    )
                }
                </div>
                <div className="view-content">
                    ...
                    {/* 一个动态路由容器 */}
                    <Route path="/animation(/:path)?"  render={props => <View {...props} />}  />
                </div>
            </React.Fragment>
        )
    }
